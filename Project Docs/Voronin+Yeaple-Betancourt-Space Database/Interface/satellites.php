...
{ display: none !important; }</style></head>
    <body class="subpage">
        ...
        <!-- Banner -->
            <section id="banner">
                <h1>These are the Satellites</h1>
            </section>
            <div class="inner">
                <?php
            ...
            $stid = oci_parse($conn, "SELECT column_name from user_tab_columns where
            ...
            $stid = oci_parse($conn, "SELECT column_name from user_tab_columns where
            table_name = 'FULL_ARTIFICIAL_SAT'");
            if (!$stid) {
                $e = oci_error($conn);
                trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
            }
            ...
            print "</thead>\n";
            $stid = oci_parse($conn,             
                'SELECT * FROM FULL_ARTIFICIAL_SAT');
            if (!$stid) {
                $e = oci_error($conn);
                trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
            }
